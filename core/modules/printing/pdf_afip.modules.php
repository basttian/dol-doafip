 ...
 */
class pdf_afip extends ModelePDFFactures
{
	/**
	 ...
	 */
	public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0 , $idFactudata )
	{
		// phpcs:enable
		...
		if ($conf->doafip->multidir_output[$conf->entity]) {
			...
			if ($object->specimen) {
				$dir = $conf->doafip->multidir_output[$conf->entity];
				$file = $dir."/SPECIMEN.pdf";
			} else {
				$objectref = dol_sanitizeFileName($object->ref);
				$dir = $conf->doafip->multidir_output[$object->entity]."/".$objectref;
			...
			if (file_exists($dir)) {
				...
				$top_shift = $this->_pagehead($pdf, $object, 1, $outputlangs, $outputlangsbis, $idFactudata);
					...
					if ($pageposafternote > $pageposbeforenote) {
						...
						while ($pagenb < $pageposafternote) {
							...
							if (empty($conf->global->MAIN_PDF_DONOTREPEAT_HEAD)) {
								$this->_pagehead($pdf, $object, 0, $outputlangs, $outputlangsbis, $idFactudata);
							}
				...
				$pdf->Output($file, 'F');
				//Send to doafip pdf new windows
				echo json_encode(array('url' => $objectref.'/'.$objectref.'.pdf' ));
				// Add pdfgeneration hook
				$hookmanager->initHooks(array('pdfgeneration'));
				$parameters = array('file'=>$file, 'object'=>$object, 'outputlangs'=>$outputlangs);
				global $action;
	 ...
	 */
	protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs, $outputlangsbis = null, $idfactudata)
	{
		...
		$pdf->setY(10);
		$pdf->setX(90);
		$txt = $objFactuData->ctetipo($objFactuData->CbteTipo);
		$pdf->SetFont('', 'B', 40);
		$pdf->SetTextColor(8, 8, 8);
		$pdf->MultiCell(25, 15, $txt, '1', 'C', 0, 0, '', '', true, 0, false, true, 0, 'M');
		$pdf->setY(28);
		$pdf->setX(90);
		$txt1 = "COD.".str_pad($objFactuData->CbteTipo, 3, "0", STR_PAD_LEFT);
		$pdf->SetFont('', 'B', 9);
		$pdf->SetTextColor(128, 0, 0);
		$pdf->MultiCell(25, 15, $txt1, '0', 'C', 0, 0, '', '', true, 0, false, true, 0, 'M');
		//FIN - CTE TIPO
		...
		$pdf->SetXY($posx, $posy);
		$pdf->SetTextColor(0, 0, 60);
		$textoAfip .= "Punto de Venta : ". str_pad($objFactuData->PtoVta, 3, "0", STR_PAD_LEFT) ." ";
		$textoAfip .= "Comp. Nro : ". str_pad($objFactuData->ref, 8, "0", STR_PAD_LEFT) ."\n";
		$textoAfip .= "Fecha de inicio de actividades : ". $conf->global->DOAFIP_MYPARAM0 ."\n";
		$pdf->MultiCell($w, 3, $textoAfip , '', 'R');
		//FIN DIBUJO DATOS AFIP
