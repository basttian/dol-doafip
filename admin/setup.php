...
/**
 * \file    doafip/admin/setup.php
 * \ingroup doafip
 * \brief   Doafip setup page.
 */
...
// Translations
$langs->loadLangs(array("admin", "doafip@doafip"));
...
$arrayofparameters = array(
	...
	//'DOAFIP_MYPARAM5'=>array('type'=>'yesno', 'enabled'=>1),
	//'DOAFIP_MYPARAM5'=>array('type'=>'thirdparty_type', 'enabled'=>1),
	//'DOAFIP_MYPARAM6'=>array('type'=>'securekey', 'enabled'=>1),
	//'DOAFIP_MYPARAM7'=>array('type'=>'product', 'enabled'=>1),
);
...
if ($action == 'updateMask') {
	...
	foreach ($dirmodels as $reldir) {
		$file = dol_buildpath($reldir."core/modules/doafip/doc/pdf_".$modele."_".strtolower($tmpobjectkey).".modules.php", 0);
		if (file_exists($file)) {
			$filefound = 1;
	...
	if (!empty($tmpobjectkey)) {
		$constforval = 'DOAFIP_'.strtoupper($tmpobjectkey)."_ADDON";
	...
	if ($ret > 0) {
		$tmpobjectkey = GETPOST('object');
		if (!empty($tmpobjectkey)) {
			$constforval = 'DOAFIP_'.strtoupper($tmpobjectkey).'_ADDON_PDF';
...
$head = doafipAdminPrepareHead();
print dol_get_fiche_head($head, 'settings', $langs->trans($page_name), -1, "doafip@doafip");
					...
					while (($file = readdir($handle)) !== false) {
						if (strpos($file, 'mod_'.strtolower($myTmpObjectKey).'_') === 0 && substr($file, dol_strlen($file) - 3, 3) == 'php') {
							...
							if ($module->isEnabled()) {
								...
								$constforvar = 'DOAFIP_'.strtoupper($myTmpObjectKey).'_ADDON';
